...
$(document).ready(function () {
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    let column = $(
      `<div class="column-name column-class-${i}" id="column-id-${ans}">${ans}</div>`
    );
  ...
  for (let i = 1; i <= 100; i++) {
    ...
    for (let j = 1; j <= 100; j++) {
      let rowCols = $(
        `<div class="input-cell" id="input-cell-id-${
          i + "-" + j
  ...
  $(".input-cell").click(function (e) {
    ...
    if (e.ctrlKey) {
      ...
      if (rowid > 1) {
        let topcellselected = $(
          `#input-cell-id-${rowid - 1}-${colid}`
        ).hasClass("selected");

        if (topcellselected) {
          $(this).addClass("top-cell-selected");
          $(`#input-cell-id-${rowid - 1}-${colid}`).addClass(
      ...
      if (colid > 1) {
        let leftcellselected = $(
          `#input-cell-id-${rowid}-${colid - 1}`
        ).hasClass("selected");

        if (leftcellselected) {
          $(this).addClass("left-cell-selected");
          $(`#input-cell-id-${rowid}-${colid - 1}`).addClass(
            "right-cell-selected"
          );
        }
      }
...
$(".icon-add").click(function () {
  ...
  $(".sheet-tab-container").append(
    `<div class="sheet-tab selected" id=#${sheetname}>${sheetname}</div>`
  );
...
function addSheetEvent() {
  ...
  $(".sheet-tab").contextmenu(function (e) {
    ...
    if ($(".sheet-option-menu").length == 1 || $(".sheet-option-menu").length == 0) {
      ...
      $(".container").append(
        '<div class="sheet-option-menu"> <div class="rename-menu">Rename </div> <div class="delete-menu">Delete </div>'
      );
      ...
      $(".delete-menu").click(function () {
        $(".container").append(`<div class="sheet-delete-modal">
            <h4 class="sheet-delete-modal-heading"> Are you sure you want to delete it ? </h4>
            <div class="delete-sheet-data">
        ...
        $(".yes-button").click(function () {
            if(Object.keys(cellData).length>1)
            {
                ...
                /*Storing the index of current sheet*/ 
                let currsheetindex=Object.keys(cellData).indexOf(selectedSheet);
                let currsheetname=selectedSheet;
                /*Storing the UI-element of current selected sheet */
                let currentsheetUI=$(".sheet-tab.selected");
                
                if(currsheetindex==0){
                    /*This just goes to the sibling and click on it*/
